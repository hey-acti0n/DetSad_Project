    ...
    <main className="home">
      <header className="home-header">
        ...
        <div className="home-title">
          <h1>Эко-сад</h1>
          <p className="subtitle">Выбери себя и играй</p>
        </div>
      </header>

      {!selected ? (
        <ChildCards children={children} onSelect={setSelected} />
      ) : (
        <div className="game-wrap">
          <div className="balance-bar">
            <span className="balance-label">Экоши</span>
            <span className="balance-value">{selectedChild?.balance ?? 0}</span>
            <Link to={`/our-tree?group=${encodeURIComponent(selectedChild?.groupId || '')}`} className="btn-tree-link">
              Наше дерево
            </Link>
            <button type="button" className="btn-back" onClick={() => setSelected(null)} aria-label="Назад">
              ← Назад
            </button>
          </div>
